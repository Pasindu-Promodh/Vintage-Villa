...
import { auth, db } from "../../onepirate/firebaseConfig";
import {
  collection,
  addDoc,
  getDocs,
...
function RoomManagement() {
  ...
  const moveRoom = async (roomIndex: number, direction: "up" | "down") => {
    ...
    try {
      ...
      setRooms(newRooms);
      setSuccess("Room order updated!");
    } catch (err) {
      console.error("Error updating room order:", err);
      setError("Failed to update room order. Please try again.");
    }
  };
  ...
  const savePricingSettings = async () => {
    try {
      ...
      setError("Failed to update pricing settings. Please try again.");
    }
  };
